...
import { deserializeMetadata, events, NodeMetadata, pluginData, serializeMetadata, InstanceInfo, ComponentUsageReport } from './types'
...
function drawComponentInstancesField(): h.JSX.Element | null {
    const isComponent = metadata != null && metadata.type === 'COMPONENT';
    const isComponentSet = metadata != null && metadata.type === 'COMPONENT_SET';

    if (!isComponent && !isComponentSet) {
        return null;
    }

    const [report, setReport] = useState<ComponentUsageReport | null>(null);
    ...
    function handleFindInstances() {
        if (!expanded) {
            ...
            setLoading(true);
            setReport(null);
            emit(events.findComponentInstances);
        } else {
            setExpanded(false);
            setReport(null);
        }
    ...
    function handleGoToInstance(instanceId: string) {
        emit(events.navigateToInstance, instanceId);
    }
    ...
    function groupByPage(instances: InstanceInfo[]): Record<string, InstanceInfo[]> {
        const groups: Record<string, InstanceInfo[]> = {};
        for (const inst of instances) {
            if (!groups[inst.pageName]) groups[inst.pageName] = [];
            groups[inst.pageName].push(inst);
        }
        return groups;
    }
    ...
    if (expanded && !loading && report != null) {
        if (report.isUnused) {
            content.push(
                <Container space='extraSmall'>
            ...
            const grouped = groupByPage(report.instances);
            const pageNames = Object.keys(grouped).sort();

            if (pageNames.length > 1) {
                for (let pageIdx = 0; pageIdx < pageNames.length; pageIdx++) {
                    const pageName = pageNames[pageIdx];
                    const pageInstances = grouped[pageName];
                    const isPageOpen = expandedPages[pageName] === true;

                    if (pageIdx > 0) {
                        content.push(<VerticalSpace space='small' />);
                    }
                    ...
                    content.push(
                        <Disclosure
                            ...
                            open={isPageOpen}
                            title={pageName + ' (' + pageInstances.length + ')'}
                        >
                            {pageItems}
                        </Disclosure>
                    );
                ...
                // Single page — no need for collapsible groups
                const pageInstances = grouped[pageNames[0]];
                for (const inst of pageInstances) {
                    content.push(
                        <div style={{ display: 'flex', alignItems: 'center' }}>
                            <IconButton onClick={() => handleGoToInstance(inst.instanceId)} title="Go to instance">
            ...
            content.push(
                <Container space='extraSmall'>
                    <VerticalSpace space='small' />
                    <Button secondary fullWidth onClick={handleRefresh}>Refresh</Button>
                    <VerticalSpace space='small' />
                </Container>
            );
        }
    ...
    const title = report != null && !loading
        ? 'Instances (' + report.totalCount + ')'
        : 'Instances';

    return (
        <Container space='extraSmall'>
            <Divider />
            <Disclosure onClick={handleFindInstances} open={expanded} title={title}>
                {content}
            </Disclosure>
        </Container>
    );
